...
// Phase 8F — authorization matrix and redaction proof for the Netlas
// routes:
//   GET  /api/findings/:id/enrichment/netlas   (read:findings)
//   POST /api/findings/:id/enrichment/netlas   (trigger:finding-enrichment)
// Mirrors shodanEnrichmentRouteAuthorization.test.js's shape: only Prisma
...
const SECRET_KEY = "SECRET-NETLAS-API-KEY-MUST-NOT-LEAK";
...
function fakeSuccessFetch() {
  return async () => ({
    ...
    json: async () => ({
      ...
      ports: [{ port: 22, prot4: "tcp", prot7: "ssh" }],
      ...
      whois: { asn: { number: 64500, name: "EXAMPLE-ASN" } },
...
beforeAll(() => {
  ...
  // Production always resolves globalThis.fetch inside netlasExecutionService.js
...
describe("GET /api/findings/:id/enrichment/netlas — read:findings", () => {
  const READ_ROLES = ["ADMIN", "ANALYST", "REVIEWER", "VIEWER"];

  it.each(READ_ROLES)("%s can read Netlas enrichment history", async (role) => {
    const res = await auth(request(app).get("/api/findings/1/enrichment/netlas"), role);
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveProperty("records");
    expect(res.body.data.provider).toBe("netlas");
  });

  it("returns 401 for an unauthenticated read", async () => {
    const res = await request(app).get("/api/findings/1/enrichment/netlas");
    expect(res.status).toBe(401);
  });

  it("returns 404 for a nonexistent finding", async () => {
    const res = await auth(request(app).get("/api/findings/999/enrichment/netlas"), "ADMIN");
    expect(res.status).toBe(404);
  });

  it("never leaks NETLAS_API_KEY in the response body", async () => {
    const res = await auth(request(app).post("/api/findings/1/enrichment/netlas"), "ADMIN").send({});
    expect(res.status).toBe(200);
    const read = await auth(request(app).get("/api/findings/1/enrichment/netlas"), "ADMIN");
    const serialized = JSON.stringify(read.body) + JSON.stringify(res.body);
    expect(serialized).not.toContain(SECRET_KEY);
  });
});

describe("POST /api/findings/:id/enrichment/netlas — trigger:finding-enrichment is ADMIN + ANALYST only", () => {
  const ALLOWED_ROLES = ["ADMIN", "ANALYST"];
  const DENIED_ROLES = ["REVIEWER", "VIEWER"];

  it.each(ALLOWED_ROLES)("%s can trigger a Netlas lookup", async (role) => {
    const res = await auth(request(app).post("/api/findings/1/enrichment/netlas"), role).send({});
    expect(res.status).toBe(200);
    ...
    expect(res.body.data.record.certificateSubject).toBe("example.com");
    expect(store.netlasEnrichments).toHaveLength(1);
  });
  ...
  it.each(DENIED_ROLES)("%s is denied with 403 and no lookup is attempted", async (role) => {
    ...
    expect(store.netlasEnrichments).toHaveLength(0);
  });

  it("returns 401 for an unauthenticated request, and triggers nothing", async () => {
    const res = await request(app).post("/api/findings/1/enrichment/netlas").send({});
    expect(res.status).toBe(401);
    expect(store.netlasEnrichments).toHaveLength(0);
  });

  it("returns 404 for a nonexistent finding, and audits nothing", async () => {
    const res = await auth(request(app).post("/api/findings/999/enrichment/netlas"), "ADMIN").send({});
    expect(res.status).toBe(404);
    expect(store.auditLogs).toHaveLength(0);
  });

  it("writes an attempted + succeeded audit pair with no secret and no raw upstream body", async () => {
    await auth(request(app).post("/api/findings/1/enrichment/netlas"), "ADMIN").send({});
    const actions = store.auditLogs.map((e) => e.action);
    expect(actions).toEqual(["netlas.lookup.attempted", "netlas.lookup.succeeded"]);
    const serialized = JSON.stringify(store.auditLogs);
    expect(serialized).not.toContain(SECRET_KEY);
    expect(serialized).not.toMatch(/"organization":"Example Org"/); // the raw upstream body, never audited
  });
});
