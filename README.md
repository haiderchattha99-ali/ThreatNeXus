...
| **AI finding assistance** | Phase 8C — Finding summary / explanation drafts, human-approved only | **No live provider ships** | Same `AI_ENABLED`/`AI_PROVIDER` switch as AI mapping assistance. Requested at `POST /api/findings/:id/ai-suggestions` (ADMIN/ANALYST), accepted/rejected at `.../accept` \| `.../reject` (ADMIN/REVIEWER). Disabled by default; offline mock for tests only. |
